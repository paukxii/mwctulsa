          ...
          <ul className='flex justify-end gap-10 sm:gap-1 md:gap-2'>
            {our_shcedules.map((schedules, index) => (
              <div className='flex mb-0' key={index}>
                <div className='sm:hidden md:hidden h-full mx-4 w-1 bg-slate-300 rounded-full rotate-6'/>
                  <div
                    key={index}
                    className='flex flex-col max-w-max whitespace-nowrap px-2'
                  >
                    <h2 className='text-mwc-red text-2xl lg:text-3xl'>
                      {schedules.day}
                    </h2>
                    {schedules.program.map((p) => (
                      <div key={p.name}>
                        ...
                        <p className='text-black capitalize text-sm'>{p.time}</p>
                      </div>
                    ))}
                  </div>
              </div>
